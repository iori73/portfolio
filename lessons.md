...
## 2026-08-16: 検証サーバーを2本立てると、片方の実測が静かに嘘になる

同じ worktree で `next start`（:3210、本番ビルドの検証用）と `next dev`（:3211、
別条件の確認用）を**同時に**動かした。dev サーバーが `.next` を書き換えるため、
`next start` 側が配信していた **`/_next/static/**` 17件が全部 400** になった。

**厄介なのは、それでも `/` が 200 を返し続けること。** サーバー描画の HTML は
出るので、ステータスコードだけ見るチェックは全部通る。実際に壊れているのは
CSS と JS だけで、しかもブラウザにキャッシュが残っていると画面も正常に見える。
気づいたのは、独立監査役の別セッションが静的アセットを1本ずつ叩いてくれたから。

### 教訓

1. **`.next` は1プロセスが専有する。** dev と start を同時に立てない。
   どうしても2条件を並べるなら worktree を分けるか `distDir` を分ける
2. **JS に依存する項目を実測したら、JS が本当に動いたことを一緒に記録する。**
   ページが 200 であることは、そのページのスクリプトが読めた証拠にならない。
   今回は測り直しのときに `document.querySelector('script[src^="/_next/"]').length` と
   `getComputedStyle(document.body).backgroundColor` を一緒に取って、
   スクリプト6本・地色 `rgb(235,232,222)` を確認してから本題を測った
3. lessons.md には既に「dev 稼働中に build しない」と書いてあった。
   **同じ罠の別の顔**で、書いてあっても踏むときは踏む。
   「`.next` を触るプロセスは常に1つ」と一段抽象化して覚えるほうがよい
...
## 2026-08-05: 「同時に2言語を並べる」UIは i18n の対象ではない

**状況**: トップページの導入部は左が英語・右が日本語という並置レイアウトなのに、
左カラムだけ `t('hero.description1')` を通していた。ロケールが jp のとき
`t()` が日本語を返すので、**左右どちらも日本語**になった。本番で数日出ていた。

**教訓**:
- `useTranslations` / `t()` は「今のロケールの1言語を出す」ための道具。
  **複数言語を同時に見せる箇所では使えない**。使うとロケールに引きずられる
- 並置レイアウトはメッセージファイルを直接 import してロケールを固定する
  （`import enMessages from '@/messages/en.json'`）。ハードコードより良いのは、
  コピーの実体がメッセージファイル1箇所に残るため
- 症状の出方が非対称だったのが見落としの原因。EN ロケールでは
  「左=EN / 右=JP」で正しく見えるので、**EN だけ見ていると気づかない**。
  バイリンガル表示のある画面は必ず全ロケールで確認する
- `curl | grep -c` での検証は誤誘導になる。RSC はアクティブロケールのメッセージ辞書を
  HTML にシリアライズするので、可視テキスト以外のヒットが混ざる。DOM を抽出して比べる
...
## 和欧を並置するときは、同じ px = 同じ見た目にならない

**やってしまったこと**: バイリンガル導入部で英語と日本語を同じ `text-body-lg`
（21px）にしていた。「同じサイズなら揃う」は和欧混植では成り立たない。

**なぜ**: Latin の見た目の大きさは x-height が決めるが、CJK は字面（グリフ本体）が
ほぼ全面を占める。21px 時で Helvetica Neue の x-height 10.9px に対し
Noto Sans JP の字面は 19.1px — **約 1.76倍**。日本語だけが大きく見え、
かつ字面が詰まった塊なので行間の空きも広く見える。

**対処**: 英語と**同じランクで同時に見せる**場合のみ、日本語を 0.85〜0.86 倍する
（`text-body-lg-jp`）。ロケール切替で片方だけ出す場面には不要 — 並置しない限り
比較対象が存在しないため。

**実装上の罠**: 補正値を px 直書きにしない。この site の型スケールは CSS 変数で
レスポンシブ（`--text-size-lg` は 19px→21px）なので、18px と書くとデスクトップだけ
正しくモバイルで比率が崩れる。`calc(var(--text-size-lg) * 0.857)` と比率で持たせる。

## タイポの案出しは本番と同じコンテンツ幅で比較する

4案を比較して「B だけ改行が文節で切れる」と結論しかけたが、比較用コンテナを
1200px で組んでいた（本番のコンテンツ幅は 1152px）。実幅で撮り直したら改行位置が
全案で変わり、その根拠は消えた。

**改行位置はビューポート幅で動くので、そもそも案の選定根拠にしてはいけない。**
比較すべきは光学的な質量バランスであって、たまたまその幅で出た改行ではない。
比較ハーネスを作るときは最初に実ページの要素幅を測ってから組むこと。

## .DS_Store は衛生問題であってセキュリティ問題ではない

指摘される前に自分で決めつけず、実測した:

- 全パス（`/.DS_Store`, `/work/.DS_Store` 等）が **HTTP 400** — Next.js が
  ドットファイルを遮断している。情報が外に出る経路が無い
- 中身も Finder の表示状態（WindowBounds / iconSize / ShowSidebar）だけで、
  ファイル名の列挙はできない
...
## 権限ルールの検証は「他の理由で塞がれていない場所」でやる

2026-08-09、追加した `Read(**/*.pem)` が効いているか確かめる際、囮を2回置き損ねた。

1. `~/.claude/jobs/.../decoy.pem` → **読めた**。`**/` はプロジェクトルート基準で
   解決されるので、プロジェクト外のファイルは元から対象外。ルールの不備ではない
2. `.next/cache/decoy.pem` → ブロックされたが、`.next/` が組み込み規則で塞がれて
   いるだけ。`.pem` ルールの証明になっていない

3回目にプロジェクト直下へ置いて決着した。**同じディレクトリの `lessons.md` は
読めて `decoy.pem` は読めない**ことを対にして確認し、初めてルール由来だと言える。

**注意**: このときのエラー文言は "File is in a directory that is denied by your
permission settings." で、ディレクトリが原因であるかのように読める。実際は
ファイルパターンでの拒否だった。文言を信じず、同ディレクトリの別ファイルが
読めるかを必ず対照実験すること。
