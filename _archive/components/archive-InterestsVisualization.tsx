// // /Users/i_kawano/Documents/portfolio/app/about/InterestsVisualization.tsx
// 'use client';
...
//       // .attr('width', width)
//       // .attr('height', height);
//       .attr('viewBox', [-width / (2 * scaleFactor), -height / (2 * scaleFactor), width, height]) // ✅ `viewBox` で拡大
//       .attr('width', '100%') // ✅ 幅を親要素にフィット
//       .attr('height', 'auto'); // ✅ アスペクト比を維持
...
//       defs.append('path').attr('id', `arcPath-${i}`).attr('d', 'M 0,0 A 40,40 0 0,1 0,0'); // 初期値はダミー; tick で更新
//     });
...
//     // 5) ノードグループ
//     const node = svg.append('g').selectAll('.node').data(nodes).join('g').attr('class', 'node').call(drag(simulation));

//     // 6) ノードの円

//     node
//       .append('circle')
//       .attr('r', 50 * scaleFactor) // ✅ 要素ごとに `scaleFactor` を適用
...
//   // `useState` を追加
// const [scaleFactor, setScaleFactor] = React.useState(1.2); // 初期値は md (デスクトップ)

// // `useEffect` でウィンドウ幅に応じて `scaleFactor` を更新
// React.useEffect(() => {
//   const updateScaleFactor = () => {
//     if (window.innerWidth < 768) {
//       setScaleFactor(1.5); // モバイル
//     } else {
...
//       .attr('width', '100%')
//       .attr('height', 'auto')
//       .style('overflow', 'hidden') // ✅ はみ出し防止
//       .style('display', 'block'); // ✅ レスポンシブ対応
...
//       )
//       .force('charge', d3.forceManyBody().strength(-400))
//       .force('center', d3.forceCenter(0, 100))
//       .force('x', d3.forceX())
//       .force('y', d3.forceY());
...
//         defs.select(`#arcPath-${i}`).attr(
//           'd',
//           `M ${n.x! - 40},${n.y!} A 40,40 0 0,1 ${n.x! + 40},${n.y!}`,
//         );
//       });
//     });
